...
export class AppsMenuManager {
    ...
    private async isApplicationPinned(name: string) : Promise<boolean> {
        ...
        const result = await ExtensionStorageProvider.loadKey("pinnedApplications");
        if (result == null) {
            return false;
        }
    ...
    async addAppAndGetButton(appName: string, imageSource: string, svgSource?: string): Promise<HTMLSpanElement[]> {
        ...
        pinButton.addEventListener("click", async () => {
            const r = await ExtensionStorageProvider.loadKey("pinnedApplications");
            let pinnedApplications = r ? r.pinnedApplications : {};
            ...
            await ExtensionStorageProvider.storeKey("pinnedApplications", pinnedApplications);
        })
